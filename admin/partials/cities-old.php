...
<!-- Le script cities-direct-api.js est chargé via wp_enqueue_script dans class-osmose-ads-admin.php -->
    
    // Charger les départements directement depuis l'API officielle
    function loadDepartments() {
        ...
        console.log('Osmose ADS: Loading departments directly from geo.api.gouv.fr...');
        
        // Appel direct à l'API officielle (CORS autorisé)
        $.ajax({
            ...
            timeout: 30000,
            beforeSend: function() {
                console.log('Osmose ADS: Sending departments request to API...');
            },
            success: function(data) {
                console.log('Osmose ADS: Departments response received:', data);
                ...
                if (Array.isArray(data) && data.length > 0) {
                    ...
                    console.log('Osmose ADS: Departments loaded successfully:', data.length, 'items');
                } else {
                    console.error('Osmose ADS: Departments error - invalid response:', data);
                    select.html('<option value=""><?php _e('Erreur: Aucun département trouvé', 'osmose-ads'); ?></option>');
                }
            },
            error: function(xhr, status, error) {
                console.error('Osmose ADS: Error loading departments from API:', {
                    status: status,
                    error: error,
                ...
                select.prop('disabled', false);
                var errorMsg = '<?php _e('Erreur de connexion à l\'API', 'osmose-ads'); ?>';
                if (xhr.status === 0) {
                    errorMsg = '<?php _e('Erreur: Impossible de se connecter à l\'API. Vérifiez votre connexion Internet.', 'osmose-ads'); ?>';
                }
    ...
    // Charger les régions directement depuis l'API officielle
    function loadRegions() {
        ...
        console.log('Osmose ADS: Loading regions directly from geo.api.gouv.fr...');
        ...
        $.ajax({
            ...
            timeout: 30000,
            beforeSend: function() {
                console.log('Osmose ADS: Sending regions request to API...');
            },
            success: function(data) {
                console.log('Osmose ADS: Regions response received:', data);
                select.prop('disabled', false);
                ...
                if (Array.isArray(data) && data.length > 0) {
                    ...
                    console.log('Osmose ADS: Regions loaded successfully:', data.length, 'items');
                } else {
                    console.error('Osmose ADS: Regions error - invalid response:', data);
                    select.html('<option value=""><?php _e('Erreur: Aucune région trouvée', 'osmose-ads'); ?></option>');
                }
            },
            error: function(xhr, status, error) {
                console.error('Osmose ADS: Error loading regions from API:', {
                    status: status,
                    error: error,
                    statusCode: xhr.status,
                    responseText: xhr.responseText
    ...
    $('#city_search').on('input', function() {
        ...
        searchTimeout = setTimeout(function() {
            // Appel direct à l'API officielle pour la recherche
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    console.error('Osmose ADS: Error searching cities:', error);
                    $('#city-search-results').hide().empty();
                }
            });
        }, 500);
