...
(function($) {
    ...
    function sendToWordPressForImport(communes, importType) {
        ...
        if (typeof osmoseAds === 'undefined') {
            console.error('osmoseAds object not defined! Creating fallback...');
            window.osmoseAds = {
                ajax_url: window.ajaxurl || '/wp-admin/admin-ajax.php',
                nonce: ''
            };
        }
        var osmoseAds = window.osmoseAds;
        
        if (!osmoseAds.ajax_url || !osmoseAds.nonce) {
            resultDiv.html('<div class="alert alert-danger">Configuration AJAX manquante. Rechargez la page.</div>');
            $('button[type="submit"]').prop('disabled', false);
            return;
        }
